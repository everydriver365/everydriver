...
function PhoneFrame({ title, subtitle, children }: { title: string; subtitle: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      <div className="w-[390px] h-[280px] rounded-[2.5rem] border-[3px] border-foreground/15 bg-[#f2f2f7] overflow-hidden shadow-xl flex flex-col">
      ...
      <div className="border-t border-black/5 bg-white/60 backdrop-blur-2xl">
        <div className="flex items-center justify-around h-16 px-1">
          {navItems.map((item, i) => {
            ...
            return (
              <div key={item.label} className="flex flex-col items-center justify-center gap-1 flex-1 h-full">
                <div className={`flex flex-col items-center gap-1 px-3 py-1.5 rounded-2xl transition-colors ${isActive ? "bg-[#0075c9]/10" : ""}`}>
                  <item.icon className="h-5 w-5" strokeWidth={isActive ? 2.5 : 2} color={isActive ? "#0075c9" : "#8e8e93"} />
                  <span className="text-[12px] font-medium" style={{ color: isActive ? "#0075c9" : "#8e8e93" }}>{item.label}</span>
                </div>
              </div>
            );
          })}
      ...
      <div className="border-t border-black/[0.06] bg-[#f8f8f8]">
        <div className="flex items-center justify-around h-16 px-1">
          {navItems.map((item, i) => {
            const isActive = i === activeIndex;
            return (
              <div key={item.label} className="flex flex-col items-center justify-center gap-1 flex-1 h-full relative">
                <item.icon className="h-5 w-5" strokeWidth={2} color={isActive ? "#0075c9" : "#c7c7cc"} />
                <span className="text-[12px] font-medium" style={{ color: isActive ? "#0075c9" : "#c7c7cc" }}>{item.label}</span>
                {isActive && <span className="absolute bottom-1.5 w-1.5 h-1.5 rounded-full bg-[#0075c9]" />}
              </div>
            );
          })}
        </div>
      ...
      <div className="border-t border-black/5 bg-[#f2f2f7]">
        <div className="flex items-center justify-around h-16 px-1">
          {navItems.map((item, i) => {
            ...
            return (
              <div key={item.label} className="flex flex-col items-center justify-center gap-1 flex-1 h-full">
                <div className={`flex items-center justify-center rounded-lg w-8 h-8 ${isActive ? "bg-[#0075c9]" : ""}`}>
                  <item.icon className="h-5 w-5" strokeWidth={isActive ? 2 : 1.8} color={isActive ? "#ffffff" : "#8e8e93"} />
                </div>
                <span className="text-[12px] font-medium" style={{ color: isActive ? "#0075c9" : "#8e8e93" }}>{item.label}</span>
              </div>
            );
