...
export default function InstructorDashcamGallery() {
  ...
  const fetchMedia = async () => {
    ...
    const instRes: any = await supabase
      .from("instructors" as any)
      .select("id")
      .eq("user_id", user.id)
      .maybeSingle();

    const instructor = instRes.data as { id: string } | null;
    if (!instructor) { setLoading(false); return; }

    const { data, error } = await supabase
      .from("dashcam_media")
      .select("id, geotab_media_file_id, media_type, file_name, duration_seconds, latitude, longitude, recorded_at, is_incident, status, thumbnail_url, device_id, created_at")
          ...
          <Card>
            <CardContent className="py-16 text-center">
              ...
              <p className="text-sm text-muted-foreground max-w-md mx-auto">
                {media.length === 0
                  ? "Dashcam clips will appear here once your Geotab device starts recording. Make sure your device is linked by your administrator."
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    ...
                    <p className="font-medium">
                      {format(new Date(selectedMedia.recorded_at), "PPp")}
                    </p>
                  </div>
                  <div>
                  ...
                  {selectedMedia.duration_seconds && (
                    <div>
                      ...
                      <p className="font-medium">
                        {Math.floor(selectedMedia.duration_seconds / 60)}m{" "}
                        {selectedMedia.duration_seconds % 60}s
                      </p>
                    </div>
                  )}
                  <div>
