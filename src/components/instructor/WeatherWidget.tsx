...
import {
  ...
  CloudLightning, Snowflake, Wind, Thermometer,
} from "lucide-react";
...
export function WeatherWidget({ weather, loading, className }: WeatherWidgetProps) {
  if (loading || !weather || weather.temperature == null) return null;
  ...
  return (
    <motion.div
      ...
      className={cn("bg-card rounded-none shadow-sm border border-border overflow-hidden", className)}
    >
