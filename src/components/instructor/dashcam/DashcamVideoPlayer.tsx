...
export function DashcamVideoPlayer({ mediaId, thumbnailUrl, className = "" }: DashcamVideoPlayerProps) {
  ...
  if (error) {
    return (
      <div className={`aspect-video bg-muted rounded-2xl flex flex-col items-center justify-center gap-2 ${className}`}>
  ...
  return (
    <div
      className={`aspect-video bg-muted rounded-2xl flex items-center justify-center cursor-pointer group relative overflow-hidden ${className}`}
      onClick={loadVideo}
    >
      {thumbnailUrl && (
