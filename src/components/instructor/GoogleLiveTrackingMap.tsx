    ...
    <div className={`relative w-full h-full ${className}`}>
      ...
      {device?.last_road_name && isConnected && (
        <div className="absolute top-16 left-4 right-4 z-20">
          <div className="bg-primary rounded-2xl px-4 py-2.5 shadow-lg text-center">
            <p className="text-sm font-bold text-primary-foreground truncate">{device.last_road_name}</p>
          </div>
        </div>
      )}
