  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-sm rounded-2xl">
        ...
        <Button onClick={handleNavigate} className="w-full gap-2 rounded-xl">
          {config.buttonLabel}
...
function PaymentsContent({ debtors }: { debtors: DebtorInfo[] }) {
  ...
  return (
    <div className="space-y-2">
      {debtors.map((d) => (
        <div key={d.id} className="flex items-center justify-between rounded-lg bg-muted/50 px-3 py-2">
