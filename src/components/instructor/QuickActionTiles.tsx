  ...
  return (
    <div className="space-y-3">
      ...
      {isEditMode ? (
        ...
        // Normal view mode — iOS home screen icon grid
              ...
              <Link key={action.id} to={action.route} className="block">
                ...
                >
                  ...
                  <div className="relative">
                    <div
                      className={`w-[60px] h-[60px] rounded-[15px] ${customIconImages[action.id] ? '' : style.iconBg} flex items-center justify-center overflow-hidden shadow-[0_2px_8px_rgba(0,0,0,0.08)]`}
                    ...
                    >
                      {customIconImages[action.id] ? (
                        ...
                        <Icon className={`h-7 w-7 ${style.iconColor}`} strokeWidth={1.6} />
                      )}
                    </div>
                    {showBadge && (
                      <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 rounded-full bg-destructive text-destructive-foreground text-[10px] font-bold flex items-center justify-center shadow-sm border-2 border-background">
                        {badgeCount > 9 ? "9+" : badgeCount}
                      </span>
                    )}
                  </div>
                  {/* Label */}
                  <span className="text-[11px] font-medium text-foreground leading-tight text-center line-clamp-1 max-w-[68px]">
                    {action.title}
                  </span>
                </motion.div>
              </Link>
