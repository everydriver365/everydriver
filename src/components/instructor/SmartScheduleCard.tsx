      ...
      <CardContent className="p-4">
        ...
        {loading ? (
          <div className="space-y-2">
            {[1, 2, 3].map(i => (
              <div key={i} className="h-12 bg-muted/50 rounded-2xl animate-pulse" />
            ))}
          </div>
        ) : (
          <div className="space-y-2">
            {suggestions.map((s, idx) => (
              <div
                key={idx}
                className="flex items-start gap-2.5 p-2.5 rounded-2xl bg-muted/30 hover:bg-muted/50 transition-colors"
              >
