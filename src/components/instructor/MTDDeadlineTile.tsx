...
export function MTDDeadlineTile({ instructorId }: MTDDeadlineTileProps) {
  ...
  if (status.loading) {
    return (
      <TileCard ariaLabel="MTD filing loading">
  ...
  if (!status.enrolled) {
    return (
      <TileCard onClick={() => navigate(SETUP_ROUTE)} ariaLabel="Get set up for Making Tax Digital">
        <div style={INNER}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{
                width: 36, height: 36, borderRadius: hasIcon3D("tax-doc") ? 0 : 10,
                background: hasIcon3D("tax-doc") ? "transparent" : "#f0edfb",
                display: "flex", alignItems: "center", justifyContent: "center",
              }}>
                {hasIcon3D("tax-doc")
                  ? <Icon3D name="tax-doc" size={36} />
  ...
  if (next.urgency === "ok") {
    ...
    return (
      <TileCard onClick={go} ariaLabel={`${next.label} MTD deadline`}>
        <div style={INNER}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={eyebrow}>MTD filing</span>
  ...
  if (next.urgency === "warning") {
    return (
      <TileCard onClick={go} accentColor="amber" ariaLabel={`${next.label} MTD deadline approaching`}>
        <div style={INNER}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
  ...
  // urgent or overdue
  return (
    <TileCard onClick={go} accentColor="red" ariaLabel={`${next.label} MTD deadline urgent`}>
      <div style={INNER}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
