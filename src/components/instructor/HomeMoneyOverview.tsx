  ...
  if (isLoading) {
    return (
      <div className="space-y-2">
        ...
        <div className="space-y-2">
          {[1, 2].map(i => (
            <div key={i} className="bg-card rounded-2xl shadow-lift border p-4 animate-pulse">
        ...
        {pupils.map((pupil, index) => (
          <Link key={pupil.id} to={`/instructor/pupils/${pupil.id}`}>
            <motion.div
              ...
              className="bg-card rounded-2xl shadow-lift border p-4 flex items-center gap-3 active:scale-[0.99] transition-transform"
            >
