      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <select className="w-full rounded-2xl border p-2 text-sm bg-background" value={newType} onChange={(e) => setNewType(e.target.value)}>
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            {showFill?.items.map((item) => (
              <button key={item.id} className="flex items-center gap-3 w-full text-left p-2 rounded-2xl hover:bg-secondary transition-colors" onClick={() => setFillResponses((r) => ({ ...r, [item.id]: !r[item.id] }))}>
