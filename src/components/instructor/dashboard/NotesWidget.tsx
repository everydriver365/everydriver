          ...
          <div className="space-y-1.5">
            {recentNotes.map((note) => {
              if (editingId === note.id) {
                return (
                  <div key={note.id} className="p-2 rounded-md border border-border space-y-2">
                    <Input
                      value={editTitle}
              ...
              return (
                <div
                  key={note.id}
                  className={`flex items-start gap-2 p-2 rounded-md hover:bg-muted/50 transition-colors group ${isDeleted ? "opacity-50" : ""}`}
                >
