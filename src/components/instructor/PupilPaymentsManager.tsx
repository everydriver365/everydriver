    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("square-checkout", {
        body: {
          ...
          returnUrl: `https://everydriver.co.uk/pay/${instructorId}?success=true`,
          cancelUrl: `https://everydriver.co.uk/pay/${instructorId}?cancelled=true`,
          instructorId,
          pupilId,
        },
      });
                ...
                {editId === p.id ? (
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      {p.payment_type === "intensive_hours" ? (() => {
                        const m = (p.notes || "").match(/(\d+(?:\.\d+)?)h used/);
                        const hrs = m ? m[1] : "?";
                        return (
