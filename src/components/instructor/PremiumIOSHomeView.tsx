...
function formatCountdown(minutes: number): string {
  ...
  if (minutes < 60 * 24) {
    ...
    return h === 1 ? "Starts in 1 hour" : `Starts in ${h} hours`;
  }
  const d = Math.round(minutes / (60 * 24));
  return d === 1 ? "Starts in 1 day" : `Starts in ${d} days`;
}
...
function NextLessonTile({
  ...
  if (!nextLesson) {
    return (
      <section className="mt-4">
        <div
          className="bg-white rounded-[22px] shadow-[0_1px_2px_rgba(16,24,40,0.04),0_8px_28px_-12px_rgba(16,24,40,0.08)] overflow-hidden relative"
          style={{ borderLeft: "3px solid #007AFF" }}
        >
          <div className="px-4 py-5 text-center">
            <div className="text-[11px] font-semibold tracking-[0.06em] text-[#3C3C43]/55 uppercase mb-2">
  ...
  const initials = (nextLesson.pupilName || "?")
    .split(" ")
    .map((n) => n[0])
    .join("")
    .slice(0, 2)
  ...
  const startHHmm = nextLesson.startTime?.slice(0, 5) || "--:--";
  const countdown = formatCountdown(nextLesson.minutesUntil);
  const isLive = nextLesson.checkInStatus === "in_progress";
    ...
    <section className="mt-4">
      ...
      >
        <button
          ...
          className="w-full text-left px-4 pt-3.5 pb-3 active:bg-black/[0.02] transition-colors"
        >
          <div className="flex items-center justify-between mb-2">
            <span className="text-[11px] font-semibold tracking-[0.06em] text-[#3C3C43]/55 uppercase">
              Next lesson
            </span>
            <span
              className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${
                isLive
                  ? "bg-[#34C759]/15 text-[#1F8E3F]"
                  : "bg-[#007AFF]/10 text-[#007AFF]"
              }`}
            >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-[16px] font-semibold tracking-tight text-[#1C1C1E] truncate">
                {nextLesson.pupilName}
              </div>
              <div className="flex items-center gap-1 mt-0.5 min-w-0">
                <MapPin className="size-[12px] text-[#3C3C43]/55 shrink-0" />
                <div className="text-[12.5px] text-[#3C3C43]/65 truncate">
                  {nextLesson.pickupLocation || nextLesson.pickupPostcode || "Standard lesson"}
                </div>
              </div>
            </div>
            <div className="text-right shrink-0">
              ...
              <div className="text-[11.5px] font-medium text-[#3C3C43]/55 mt-1 tabular-nums">
                {formatDuration(nextLesson.durationMinutes)}
              </div>
            </div>
            <ChevronDown
              className={`size-[16px] text-[#3C3C43]/40 shrink-0 transition-transform ${expanded ? "rotate-180" : ""}`}
            />
          </div>
        </button>
        ...
        {expanded && (
          <div className="px-4 pb-4 pt-1">
            {nextLesson.lastLessonPlan && (
              <div className="rounded-[12px] bg-[#F2F2F7] px-3 py-2.5 mb-3">
                <div className="text-[11px] font-semibold tracking-wide text-[#3C3C43]/55 uppercase mb-1">
                  Plan
                </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              <button
                onClick={onCall}
                disabled={!nextLesson.pupilPhone}
                className="flex flex-col items-center justify-center gap-1 rounded-[12px] bg-[#34C759]/[0.10] text-[#1F8E3F] py-2.5 active:bg-[#34C759]/[0.18] disabled:opacity-40 transition-colors"
              >
              ...
              <button
                onClick={onNavigate}
                disabled={!nextLesson.pickupPostcode}
                className="flex flex-col items-center justify-center gap-1 rounded-[12px] bg-[#007AFF]/[0.10] text-[#007AFF] py-2.5 active:bg-[#007AFF]/[0.18] disabled:opacity-40 transition-colors"
              >
                <Navigation className="size-[16px]" />
                <span className="text-[12px] font-semibold">Navigate</span>
              </button>
              <button
                onClick={onOpen}
                className="flex flex-col items-center justify-center gap-1 rounded-[12px] bg-[#5856D6]/[0.10] text-[#5856D6] py-2.5 active:bg-[#5856D6]/[0.18] transition-colors"
              >
