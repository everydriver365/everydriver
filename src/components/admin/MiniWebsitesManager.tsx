...
export function MiniWebsitesManager() {
  ...
  const fetchWebsites = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from("instructors")
        .select("id, name, email, app_slug, website_theme, custom_domain, custom_domain_verified, is_active, created_at, brand_colour")
  ...
  const getWebsiteUrl = (slug: string | null) => {
    if (!slug) return null;
    return `https://drive365.co.uk/${slug}`;
  };
                  ...
                  <TableCell>
                    {site.app_slug ? (
                      <div className="flex items-center gap-2">
                        ...
                        <a
                          href={getWebsiteUrl(site.app_slug) || "#"}
                          target="_blank"
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        {site.app_slug && (
                          <DropdownMenuItem 
                            onClick={() => window.open(getWebsiteUrl(site.app_slug), "_blank")}
                          >
                        ...
                        <DropdownMenuItem 
                          onClick={() => window.open(`/admin?section=instructors&id=${site.id}`, "_blank")}
                        >
