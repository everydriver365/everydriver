...
export function InstructorPortalLayout({ children, hideHeader }: InstructorPortalLayoutProps) {
  ...
  const mobileBg = wallpaperColor || "#F4F7F6";
    ...
    return (
      ...
      >
        ...
        {isFullscreenMode ? (
          ...
          <>
            <OfflineBanner />
            <main className={`ios-scroll ${hideHeader ? '' : location.pathname === '/instructor' ? '' : 'px-4 py-4'}`} style={{ backgroundColor: hideHeader ? 'transparent' : isHomePage ? 'transparent' : '#F2F2F7' }}>{children}</main>
