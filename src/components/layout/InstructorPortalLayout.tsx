...
function HomeStatsCard({ instructorId }: { instructorId: string | undefined }) {
  const { data: weeklyGoals } = useWeeklyGoals(instructorId);
  const lessonsToday = weeklyGoals?.lessonsScheduled ?? 0;
  const lessonsCompleted = weeklyGoals?.lessonsCompleted ?? 0;
  const hoursThisWeek = weeklyGoals?.hoursThisWeek ?? 0;
  const hoursGoal = weeklyGoals?.hoursGoal ?? 30;
  const progressPercent = Math.min(100, Math.round((hoursThisWeek / (hoursGoal || 1)) * 100));
  const r = 22, stroke = 4, c = 2 * Math.PI * r;
  const offset = c * (1 - progressPercent / 100);

  return (
    <div className="px-4 -mt-7 relative z-10 mb-2">
      <div className="bg-card rounded-2xl p-3.5 flex items-center justify-between border border-border/50" style={{ boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[15px] font-semibold text-foreground mt-0.5 leading-snug">
            {lessonsCompleted} of {lessonsToday} lessons done
          </p>
          <p className="text-[12px] text-muted-foreground mt-0.5">
            {hoursThisWeek}h this week · {hoursGoal}h goal
          </p>
        </div>
        <div className="relative w-[52px] h-[52px] shrink-0">
          <svg viewBox="0 0 52 52" className="w-full h-full -rotate-90">
            <circle cx="26" cy="26" r={r} fill="none" stroke="currentColor" strokeWidth={stroke} className="text-border" opacity={0.3} />
            <circle cx="26" cy="26" r={r} fill="none" stroke="#34D399" strokeWidth={stroke} strokeLinecap="round" strokeDasharray={c} strokeDashoffset={offset} />
          </svg>
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-sm font-bold text-foreground leading-none">{progressPercent}%</span>
          </div>
        </div>
      </div>
    </div>
...
export function InstructorPortalLayout({ children }: InstructorPortalLayoutProps) {
  ...
  // Get page title from current path (used in mobile header)
  const getPageTitle = () => {
      ...
      >
        {!isFullscreenMode && (
          <>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    {showBackButton ? (
                      <>
                        ...
                        <p className="text-sm font-semibold leading-tight truncate">{getPageTitle()}</p>
                      </>
                    ) : (
                      <>
                                ...
                                return (
                                  ...
                                  >
                                    <span className="relative">
                                      <link.icon className={cn("h-5 w-5", isHighlighted && !isActive && "text-emerald-500")} />
                                      {isAdminChat && !isActive && <AdminMessageBadge />}
                                    </span>
                                    {link.label}
                                    {isVisitorChats && !isActive && <VisitorChatBadge instructorId={instructor?.id} className="ml-auto" />}
                                    {isMessages && !isActive && <MessageNotificationBadge instructorId={instructor?.id} className="ml-auto" />}
                                    {isPendingScheduling && !isActive && <PendingSchedulingBadge instructorId={instructor?.id} className="ml-auto" />}
                                  </button>
                                );
                              })}
                            </nav>
                        ...
                        <div className="min-w-0">
                          <h1 className="text-[15px] font-bold leading-tight">{getGreeting()}, {instructor?.name?.split(" ")[0] || "there"}</h1>
                          <p className="text-[11px] text-primary-foreground/50">{format(new Date(), "EEEE d MMMM")}</p>
                        </div>
                      </>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <Button
                      ...
                      className="h-8 px-3 shrink-0 rounded-full text-xs font-semibold bg-primary-foreground/90 hover:bg-primary-foreground text-primary border-0"
                      title="Take Payment"
                    >
                    ...
                    <DropdownMenu modal={false}>
                      ...
                      <DropdownMenuContent align="end" className="w-48 bg-popover border shadow-lg z-50">
                        <DropdownMenuItem onClick={() => { setMobileSearchOpen(prev => !prev); setMobileSearchQuery(""); setMobileSearchResults([]); }} className="cursor-pointer">
            ...
            {/* Floating Stats Card (homepage only) */}
            {isHomePage && <HomeStatsCard instructorId={instructor?.id} />}

            {/* Mobile Search Overlay */}
            {mobileSearchOpen && (
              <div className={cn("sticky z-30 bg-background border-b border-border shadow-md px-3 py-3", isHomePage ? "top-[calc(env(safe-area-inset-top)+64px)]" : "top-[calc(env(safe-area-inset-top)+56px)]")}>
  ...
  // getPageTitle already defined above
