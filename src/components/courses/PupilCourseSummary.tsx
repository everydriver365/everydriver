/**
 * PupilCourseSummary — desktop course summary page (Phase 2).
 *
 * Adds on top of Phase 1:
 *  - Inline editing of pickup address, what3words, test date/time, transmission,
 *    course status, custom hourly rate, notes.
 *  - Refund flow per payment (calls record-payment edge function with isRefund).
 *  - Course duplication: copies every future lesson +7 days forward.
 *  - Live activity log feed sourced from course_activity_log (+ payment_history
 *    merged in for context).
 */
...
type Pupil = {
  ...
  transmission_type: string | null; special_needs: string | null;
...
type Lesson = {
  id: string; lesson_date: string; start_time: string;
...
type Instructor = {
  id: string; name: string; email: string | null; phone: string | null;
  profile_image_url: string | null; hourly_rate: number | null;
  school_skim_percentage: number | null;
};

type ActivityRow = {
  id: string; created_at: string; action: string;
  actor_name: string | null; details: Record<string, unknown>;
};
...
function StatusBadge({ status }: { status: string | null }) {
  ...
  return (
    <span style={{ backgroundColor: v.bg, color: v.fg, fontSize: 11, fontWeight: 700, padding: "3px 9px", borderRadius: 999 }}>
      {v.label}
    </span>
  );
}
...
function PaymentMethodPill({ method }: { method: string | null }) {
  ...
  const c =
    m.includes("klarna") ? { bg: "#FCE7F3", fg: "#9D174D" } :
    m.includes("clearpay") ? { bg: "#D1FAE5", fg: "#059669" } :
    m.includes("cash") ? { bg: "#FEF3C7", fg: "#B45309" } :
    m.includes("gocardless") || m.includes("bank") ? { bg: "#DBEAFE", fg: "#1E4D9B" } :
    m.includes("square") || m.includes("sumup") || m.includes("card") ? { bg: "#F3F4F6", fg: "#374151" } :
    { bg: "#F3F4F6", fg: "#6B7280" };
  return (
    <span style={{ backgroundColor: c.bg, color: c.fg, fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 999 }}>
      {method || "—"}
    </span>
  );
}
...
function describeActivity(row: ActivityRow): string {
  ...
  switch (row.action) {
    case "field_edited":
      return `Updated ${String(d.field ?? "field")}${d.from !== undefined ? ` (was "${d.from ?? "—"}")` : ""}`;
    case "refund_issued":
      return `Issued refund of ${fmt(Number(d.amount || 0))}`;
    case "course_duplicated":
      return `Duplicated course (+${d.lessons_added ?? 0} lessons)`;
    case "reminder_sent":
...
export function PupilCourseSummary({ pupilId, onBack, backHref }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: instructorRow }, { data: lessonRows }, { data: paymentRows }, { data: activityRows }] = await Promise.all([
      ...
      supabase.from("course_activity_log")
        .select("id,created_at,action,actor_name,details")
        .eq("pupil_id", pupilId)
    ...
    const { data: reviewRows } = await supabase
      .from("course_reviews").select("rating")
      .eq("instructor_id", pupilRow.instructor_id).eq("is_visible", true);
    if (reviewRows && reviewRows.length > 0) {
      const avg = reviewRows.reduce((a, r) => a + (r.rating || 0), 0) / reviewRows.length;
  ...
  const totals = useMemo(() => {
    const totalCollected = payments.filter(p => (p.amount || 0) > 0).reduce((a, p) => a + Number(p.amount || 0), 0);
    const totalRefunded = payments.filter(p => (p.amount || 0) < 0).reduce((a, p) => a + Math.abs(Number(p.amount || 0)), 0);
    ...
    return { totalCollected, totalRefunded, schoolProfit, paidToInstructor, attendedCount, totalLessons, totalHours, attendancePct, outstanding };
  ...
  const updatePupilField = useCallback(async (field: keyof Pupil, newValue: string | null) => {
    ...
    setPupil({ ...pupil, [field]: newValue } as Pupil);
  ...
  const handleRefund = useCallback(async (payment: Payment) => {
    if (!pupil) return;
    const max = Number(payment.amount);
    const input = window.prompt(`Refund amount (max ${fmt(max)})`, max.toFixed(2));
    if (!input) return;
    ...
    try {
      const { data, error } = await supabase.functions.invoke("record-payment", {
        body: {
          pupilId: pupil.id,
          amount,
          method: (payment.payment_method || "").toLowerCase().includes("cash") ? "cash" : "bank",
          isRefund: true,
          note: `Refund of payment ${payment.id.slice(0, 8)}`,
        },
      });
      if (error || (data as { error?: string })?.error) {
        throw new Error(error?.message || (data as { error?: string })?.error || "Refund failed");
      }
      await logCourseActivity({
        pupilId: pupil.id,
        instructorId: pupil.instructor_id,
        action: "refund_issued",
        details: { amount, original_payment_id: payment.id, method: payment.payment_method },
      });
      toast({ title: "Refund issued", description: fmt(amount) });
      load();
    } catch (e) {
  ...
  const handleDuplicate = useCallback(async () => {
    ...
    const future = lessons.filter(l => l.lesson_date >= todayIso && (l.status || "").toLowerCase() !== "cancelled");
    if (future.length === 0) {
      toast({ title: "Nothing to duplicate", description: "No future lessons on this course.", variant: "destructive" });
      return;
    }
    if (!window.confirm(`Duplicate ${future.length} lesson${future.length === 1 ? "" : "s"} +7 days forward?`)) return;

    setWorking("duplicate");
    try {
      // Fetch full rows for the lessons being duplicated so we copy every field.
      const { data: fullRows, error: fetchErr } = await supabase
        .from("scheduled_lessons").select("*").in("id", future.map(l => l.id));
      if (fetchErr) throw fetchErr;

      const inserts = (fullRows || []).map((r: Record<string, unknown>) => {
        const copy: Record<string, unknown> = { ...r };
        delete copy.id; delete copy.created_at; delete copy.updated_at; delete copy.deleted_at;
        delete copy.google_event_id; delete copy.last_synced_at;
        const d = new Date(String(copy.lesson_date)); d.setDate(d.getDate() + 7);
        copy.lesson_date = d.toISOString().slice(0, 10);
        copy.status = "scheduled";
        copy.payment_status = "unpaid";
        return copy;
      });
      const { error: insErr } = await supabase.from("scheduled_lessons").insert(inserts as never);
      ...
      await logCourseActivity({
        ...
        details: { lessons_added: inserts.length, shift_days: 7 },
      });
      toast({ title: "Course duplicated", description: `${inserts.length} lessons added.` });
      load();
    } catch (e) {
      toast({ title: "Duplication failed", description: (e as Error).message, variant: "destructive" });
    } finally {
      setWorking(null);
    }
  ...
  if (!pupil) return <div className="p-6 text-sm text-muted-foreground">Course / pupil not found.</div>;

  const attendanceColour = totals.attendancePct >= 100 ? "#059669" : totals.attendancePct >= 75 ? "#B45309" : "#C0271F";
    ...
    <div className="p-6 space-y-4" style={{ backgroundColor: "#F4F7F6", minHeight: "100%" }}>
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: "320px 280px 1fr 220px" }}>
        ...
        <div className="space-y-4">
          <Card>
            <CardHeader className="bg-transparent pb-3"><CardTitle className="text-base">Course summary</CardTitle></CardHeader>
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              <div className="pt-2 border-t">
                ...
                <InlineEditField
                  ...
                  onSave={(v) => updatePupilField("notes", v || null)}
                />
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="bg-transparent pb-3 flex flex-row items-center justify-between space-y-0">
              ...
              {rating ? (
                <span style={{ fontSize: 12, fontWeight: 600, color: "#B45309" }} className="flex items-center gap-1" title="Average instructor rating">
            ...
            <CardContent className="space-y-3">
              ...
              {(pupil.special_needs || pupil.medical_notes) && (
                <div style={{ backgroundColor: "#FFFBEB", borderLeft: "3px solid #F59E0B", borderRadius: 6, padding: "8px 10px", fontSize: 11 }} className="text-amber-900">
                  <strong>Accessibility:</strong> {[pupil.special_needs, pupil.medical_notes].filter(Boolean).join(" · ")}
                </div>
              )}
        ...
        <div className="space-y-4">
          <Card>
            <CardHeader className="bg-transparent pb-3"><CardTitle className="text-base">Instructor</CardTitle></CardHeader>
            <CardContent className="space-y-3 text-sm">
              {instructor ? (
                <>
                  <div className="flex items-center gap-3">
                    <div style={{ width: 40, height: 40, borderRadius: "50%", backgroundColor: "#1E4D9B", color: "white", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 14 }}>
        ...
        {/* Col 3 — Balance + Payments */}
        <div className="space-y-4">
          <Card>
            <CardHeader className="bg-transparent pb-3"><CardTitle className="text-base">Course balance</CardTitle></CardHeader>
            <CardContent>
              ...
              {totals.totalRefunded > 0 && (
                <div className="mt-3 text-xs" style={{ color: "#C0271F" }}>Total refunded: {fmt(totals.totalRefunded)}</div>
              )}
            </CardContent>
          </Card>

          <Card>
            <CardHeader className="bg-transparent pb-3"><CardTitle className="text-base">Payment history</CardTitle></CardHeader>
            <CardContent>
              {payments.length === 0 ? (
        ...
        <div>
          <Card>
            <CardHeader className="bg-transparent pb-3"><CardTitle className="text-base">Actions</CardTitle></CardHeader>
            <CardContent className="space-y-2">
              <Button
                ...
                size="sm"
              >
                {working === "duplicate" ? <Loader2 className="h-3.5 w-3.5 animate-spin mr-2" /> : <Copy className="h-3.5 w-3.5 mr-2" />}
                Duplicate course
              </Button>
      ...
      <Card>
        ...
        <CardContent>
          {activity.length === 0 ? (
            <div className="text-sm text-muted-foreground">No activity yet — actions will appear here from now on.</div>
          ) : (
            <div className="divide-y max-h-[260px] overflow-y-auto">
              {activity.map((a) => (
                <div key={a.id} className="py-2 flex items-start gap-3 text-sm">
                  <div className="text-[11px] text-muted-foreground" style={{ minWidth: 110 }}>{fmtDateTime(a.created_at)}</div>
                  <div className="flex-1 min-w-0">
                    <div className="text-xs font-medium">{describeActivity(a)}</div>
                    {a.actor_name && <div className="text-[10px] text-muted-foreground">by {a.actor_name}</div>}
                  </div>
                </div>
              ))}
            </div>
...
function EditRow({
  label, value, onSave, type, placeholder,
}: {
