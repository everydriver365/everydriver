...
interface TraccarConnectionStatus {
  isConnected: boolean;
...
export function useTraccarConnectionStatus(instructorId: string | null): TraccarConnectionStatus {
  ...
  const checkConnection = useCallback(async () => {
    ...
    try {
      ...
      console.error("Error checking Traccar connection:", err);
    } finally {
      setIsLoading(false);
    }
  }, [instructorId]);
