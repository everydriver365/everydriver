...
serve(async (req) => {
  ...
  try {
    ...
    const { data: abandonedCheckouts, error } = await supabase
      ...
      .is("converted_at", null).is("reminder_sent_at", null)
      ...
      try {
        ...
        if (checkout.pupil_phone && TWILIO_ACCOUNT_SID && TWILIO_AUTH_TOKEN && TWILIO_PHONE_NUMBER) {
          ...
          await fetch(`https://api.twilio.com/2010-04-01/Accounts/${TWILIO_ACCOUNT_SID}/Messages.json`, {
            ...
            body: new URLSearchParams({ To: checkout.pupil_phone, From: TWILIO_PHONE_NUMBER, Body: message }),
          });
        }

        if (checkout.pupil_email) {
          await sendBrandedEmail({
            to: checkout.pupil_email,
            subject: "Complete Your Driving Lesson Booking",
            heading: "Finish your booking",
            intro: `Hi ${checkout.pupil_name || "there"}, you started booking lessons with ${instructorName} but didn't complete checkout.`,
            paragraphs: ["Don't miss out — secure your spot today!"],
            ctaLabel: "Complete booking",
            ctaUrl: "https://everydriver.co.uk",
            idempotencyKey: `abandoned-${checkout.id}`,
          }, supabase);
        }
