...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Build callback URL
    const callbackUrl = `${supabaseUrl}/functions/v1/payment-callback?provider=npi&pupilId=${body.pupilId || ""}&ref=${orderReference}${body.type === "balance" ? "&type=balance" : ""}`;
    ...
    const signature = await createCardstreamSignature(requestData, merchantSecret);
