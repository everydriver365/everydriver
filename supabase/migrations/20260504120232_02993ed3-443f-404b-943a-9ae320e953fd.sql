...
-- Race-proof the lesson clash check.
-- The existing prevent_lesson_clash() function does a SELECT then INSERT but
-- holds no lock, so two simultaneous inserts for the same instructor/day can
-- both pass and both succeed. We add a transaction-level advisory lock on
-- (instructor_id, lesson_date) so concurrent writers serialise on the same
-- (instructor, day) pair, and re-check after taking the lock.
...
BEGIN
  ...
  -- Take a transaction-scoped advisory lock keyed on this instructor + day so
  -- concurrent inserts for the same slot serialise instead of both passing the
  -- SELECT below.
