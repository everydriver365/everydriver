...
AS $function$
  -- Returns every active scheduled lesson in range so the public availability
  -- engine can block the time, even when no pickup/dropoff coordinates exist.
  -- Coordinates remain optional and are used purely for travel padding.
  SELECT
    sl.instructor_id,
    sl.lesson_date,
    sl.start_time,
