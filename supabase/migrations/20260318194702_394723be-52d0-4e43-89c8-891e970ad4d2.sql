...
AS $function$
DECLARE
  allowed_fields TEXT[] := ARRAY['profile_image_url','date_of_birth','driver_number','theory_cert_number','phone','email','address','postcode','pickup_address','what3words','parent_portal_enabled','reminder_preferences','emergency_contact_name','emergency_contact_phone','medical_notes','preferred_duration_minutes','communication_preference'];
  field_name TEXT;
  ...
  EXECUTE format(
    ...
    (SELECT string_agg(format('%I = ($2->>%L)::%s',
      ...
      CASE key
        ...
        WHEN 'reminder_preferences' THEN 'JSONB'
        WHEN 'preferred_duration_minutes' THEN 'INTEGER'
        ELSE 'TEXT'
      END
    ), ', ')
